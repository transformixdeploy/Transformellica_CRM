...
from helpers import is_social_media_url, extract_domain, extract_username_from_url
    ...
    async def analyze_social_url(self, url: str) -> Dict[str, Any]:
        ...
        if platform == "TikTok":
            try:
                ...
                avg_likes = (sum(like_values) / len(like_values)) if like_values else 0
                ...
                engagement_rate = (engagement_per_post / followers) if followers and followers > 0 else 0
                hashtag_list = []
                for p in posts:
                    for h in (p.get('hashtags') or []):
                        hashtag_list.append(h)
                unique_hashtags = list({h for h in hashtag_list})
                return {
                    "url": url,
                    "platform": platform,
                    "is_social": True,
                    "title": f"TikTok: @{data.get('profile', {}).get('username', username)}",
                    "description": data.get('profile', {}).get('bio'),
                    "profile_data": {
                        "name": data.get('profile', {}).get('name'),
                        "bio": data.get('profile', {}).get('bio'),
                        "follower_count": data.get('profile', {}).get('followers'),
                        "following_count": data.get('profile', {}).get('following'),
                        "likes_total": data.get('profile', {}).get('likes_total'),
                    },
                    "content_analysis": {
                        "recent_posts": posts,
                        "post_count_scanned": data.get('post_count_scanned', 0),
                    ...
                    "accessibility": True,
                    "detailed_data": data,
                    "analysis_method": "selenium_scrape",
                    "limitations": [
                        "May require Chrome/Chromedriver availability on host",
                        "TikTok structure can change breaking selectors"
                    ]
                }
            except Exception as e:
